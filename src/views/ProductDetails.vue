...
<template>
  ...
  <div class="flex gap-2">
    <img
      ...
      class="w-16 h-16 object-contain rounded-xl border border-gray-200 cursor-pointer hover:border-gray-900"
    />
  </div>
    ...
    <!-- right: details -->
    <div class="flex-1">
      <p class="text-sm text-gray-400 mb-1">{{ product.category }}</p>
      <h1 class="text-3xl font-black text-gray-900 mb-3">{{ product.title }}</h1>
      <p class="text-gray-500 mb-6">{{ product.description }}</p>
      <p class="text-3xl font-black text-gray-900 mb-6">${{ product.price }}</p>
      <p class="text-sm text-gray-400 mb-6">⭐ {{ product.rating }} / 5</p>
      <!-- availability & stock -->
<div class="grid grid-cols-2 gap-3 mb-4">
  <div class="border border-gray-200 rounded-xl p-3">
    <p class="text-xs text-gray-400 mb-1">Availability</p>
    <p class="text-sm font-semibold text-green-600">{{ product.availabilityStatus }}</p>
  </div>
  <div class="border border-gray-200 rounded-xl p-3">
...
<div class="grid grid-cols-2 gap-3 mb-4">
  <div class="border border-gray-200 rounded-xl p-3">
    ...
    <p class="text-sm font-semibold text-gray-900">{{ product.shippingInformation }}</p>
  </div>
  <div class="border border-gray-200 rounded-xl p-3">
    <p class="text-xs text-gray-400 mb-1">Returns</p>
    <p class="text-sm font-semibold text-gray-900">{{ product.returnPolicy }}</p>
  </div>
</div>
...
<button @click="addProductToCart" 
 ...
 <!-- reviews section -->
<div class="mt-10">
  <h2 class="text-xl font-black text-gray-900 mb-6">Customer Reviews</h2>
  <div class="grid grid-cols-3 gap-4">
    ...
    >
      <p class="font-semibold text-gray-900 mb-1">{{ review.reviewerName }}</p>
      <p class="text-xs text-gray-400 mb-2">{{  new Date(review.date).toLocaleDateString() }}</p>
      <p class="text-sm text-gray-400 mb-2">⭐ {{ review.rating }} / 5</p>
